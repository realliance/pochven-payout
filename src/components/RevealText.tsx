...
export function RevealText({
  ...
  const [decodedList, setDecodedList] = useState<boolean[]>(
  ...
  useEffect(() => {
    if (decodedList.includes(false)) {
      ...
      const delay = allFalse
        ? (initialDelay ?? 1000) + cachedInverval
        : cachedInverval;
      const id = setTimeout(() => {
        const newDecodedList = [...decodedList];
        const toDecode = newDecodedList
          .map((val, index) => [val, index])
          .filter(([val]) => !val);

        const [_, index] =
          toDecode[Math.floor(Math.random() * toDecode.length)];
  ...
  const decodingText = useMemo(
    () =>
      cachedText.split("").map((char, index) => (
        <span
          key={index}
          className={`${decodedList[index] ? "mono-one" : "triangle"}`}
        >
          {char}
        </span>
      )),
    [decodedList, cachedText],
  );
