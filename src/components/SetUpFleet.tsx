...
export function SetUpFleet({ currentFleetMembers, setFleetMembers }: SetupFleetProps) {
    ...
    const importContent = useMemo(() => (
        <>
            <FaUsers />
            <span className="pl-3">Import {fleetMembers?.length} Fleet Members</span>
        </>
    ), [fleetMembers]);
    ...
    const importMembersFromList = async () => {
        if (token) {
            ...
            const newFleetMembers: FleetMember[] = ids.characters?.map((char) => {
                return {
                    characterId: char.id!,
                    name: char.name!,
                    eligible: true,
    ...
    const autoImportButtonContent = loading ? loadingButtonContent : isFleetCommander ? importContent : notOfRole;

    return (
        <>
            <RevealText text="Configure the Fleet" className="text-2xl" />
            <Button outline gradientDuoTone="pinkToOrange" disabled={!isFleetCommander}>{autoImportButtonContent}</Button>
            <div className="text-sm flex flex-row gap-1 text-gray-400 hover:cursor-pointer" onClick={reload}>
                <FaArrowRotateRight className="w-3" />
                <span className="underline">Refresh Fleet Status</span>
            </div>
            <div className="w-full flex flex-col gap-2">
                <div className="block">
                    <Label htmlFor="memberImport" value="Import Members" className="text-xl" />
                </div>
                <Textarea id="memberImport" placeholder={EXAMPLE_FLEET} required rows={6} value={importText} onInput={(e) => setImportText(e.currentTarget.value)} />
                <Button outline gradientDuoTone="pinkToOrange" fullSized disabled={importLoading} onClick={importMembersFromList}>{importLoading && <Spinner size="sm" />} Import</Button>
            </div>
        </>
    )
}