...
export async function beginAuthFlow() {
  ...
  authorizationUrl.searchParams.set('scope', 'esi-wallet.read_character_wallet.v1 esi-fleets.read_fleet.v1 esi-ui.open_window.v1 esi-characters.read_chat_channels.v1')
...
export async function onRedirect(updateToken: (token: string) => void) {
  ...
  const parameters = oauth.validateAuthResponse(authServer, client, currentUrl, localStorageState)
  ...
  const result = await oauth.processAuthorizationCodeOAuth2Response(authServer, client, response)
