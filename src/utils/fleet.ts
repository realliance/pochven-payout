...
export function useFleetAPI(): FleetContext {
    ...
    const [userFleet, setUserFleet] = useState<CharacterFleet | undefined>(undefined);
    const [fleetMembers, setFleetMembers] = useState<FleetMembers | undefined>(undefined);
    ...
    useEffect(() => {
        const run = async () => {
            if (loading && token && identity?.id) {
                ...
                if (characterFleet.fleet_id && characterFleet.role === "fleet_commander") {
                    setAutoMode(true);
                    const { data, error } = await getFleetMembers(token, characterFleet.fleet_id);
                    if (error) {
                        setAutoMode(false);
                        console.error(error);
                        return;
